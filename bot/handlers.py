...
async def start_handler(
    ...
    await message.answer(
        ...
        "За 2 минуты вы оцените состояние вашей системы "
...
async def process_s(
    ...
    await callback.message.answer(
        ...
        "[ 0 — Одиночество, «второй фронт» ] ↔ [ 10 — Мощная поддержка и тепло ]",
        reply_markup=rating_keyboard(0, 10)
    )
# ============================================================
# O
...
async def process_o(
    ...
    await callback.message.answer(
        ...
        "Насколько у вас есть понятные инструменты, навыки"
        "и стратегия (без необходимости тушить пожары 24/7)? "
...
async def process_l(
    ...
    await callback.message.answer(
        "А теперь оцените «барьеры» — то, что незаметно тормозит движение:"
        "N•	N (Негативные убеждения):"
...
async def process_f(
    ...
    await callback.message.answer(
        "•	H (Привычки): Насколько часто вы наступаете "
        "на одни и те же грабли в кризисных ситуациях? "
